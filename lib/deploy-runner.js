...
if (system.args.length < 2) {
    console.log('Usage: casperjs deploy-runner.js <casperPath> --url=<base-url> --file=<file> --user=<user> --pass=<pass> --module=<module-name> --log=<log-dir>');
    phantom.exit(64);
}
...
// prints opencms reports in iframe to file and stdout
var updateReport = function () {
    ...
    if (openCmsLog) {
        ...
        if (logDir) {
            var logName = logDir + '/' + dateToYMD(new Date()) + '_' + module + '.log';
            fs.write(logName, openCmsLog);
            return logName;
        }
    }
    return null;
...
// checks opencms reports for exceptions
var checkReport = function () {
    var openCmsLog = casper.evaluate(function() {
        return document.querySelector('#report').contentDocument.querySelector('.main').innerText;
    });
    if (openCmsLog) {
        var logs = openCmsLog.split('\n');
        var error = [];
        logs.forEach(function (line) {
            if (line.indexOf('Exception') > -1) {
                error.push(line.replace(/\( [0-9]+ \/ [0-9]+ \) /, ''));
            }
        });
        return error;
    }
    return['something went wrong, no log was found,'];
};
...
casper.run(function() {
    ...
    var l = updateReport();
    // check for errors
    var error = checkReport();
    if (error.length > 0) {
        // if we got an error, die gracefully with a message
        console.log(error.join('\n'));
        if (l) {
            console.log('See more in ' + l + '\n');
        }
        casper.die(error.join('\n'), 1);
    }
    else {
        // we are done
